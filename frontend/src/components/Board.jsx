    ...
    return (
        ...
        { loadingBoards ? (
            ...
            <div className="flex flex-col gap-4 bg-white border border-slate-200/50 w-full rounded-lg p-6 duration-300 hover:-translate-y-0.5">
                ...
                <button onClick={() => setEditingBoard(true)}><i className="fa-solid fa-pencil text-cyan-700 cursor-pointer text-xl hover:text-cyan-900"></i></button>
                <button onClick={() => handleDeleteClick(board._id)}><i className="fa-solid fa-x text-xl cursor-pointer text-rose-500 hover:text-rose-700"></i></button>
                
                </div>
                <p className="text-xl font-medium mt-2">{board.name}</p>
                <p className="text-sm">{board.category}</p>
                <Link to={`/board/${board._id}`} className="font-medium w-fit text-cyan-700 after:bg-cyan-700 after:w-0 hover:after:w-full after:h-0.5 after:block">View board</Link>
            </div>
        ) : (
            <>
            ...
            <div className="flex items-center justify-center bg-white/80 backdrop-blur-[2px] fixed inset-0 z-60 p-6 h-screen">

            <div className="bg-gray-100 border z-40 border-gray-400/50 flex flex-col gap-4 justify-center p-6 rounded-lg max-h-[90vh]">
            ...
            <button onClick={() => setEditingBoard(false)} className="cursor-pointer text-cyan-700 after:bg-cyan-700 after:w-0 hover:after:w-full after:h-0.5 after:block">Cancel</button>
            <button onClick={() => handleUpdateClick(board._id)} className="cursor-pointer text-cyan-700 after:bg-cyan-700 after:w-0 hover:after:w-full after:h-0.5 after:block">Update</button>
            </div>
            </div>
